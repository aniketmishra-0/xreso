...
import { getCategories, getTrendingNotes } from "@/lib/db/queries";
...
function CategoryGlyph({
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <Icon size={prominent ? 26 : 22} color={color} />
      <span className={styles.categoryGlyphLabel}>{label}</span>
    </div>
  );
}
...
export default async function HomeSecondPage() {
  const [trendingData, categories] = await Promise.all([
    getTrendingNotes(6),
    getCategories(9),
  ]);
                ...
                <div className={styles.categorySummary}>
                  <div className={styles.summaryCard}>
                    <span className={styles.summaryLabel}>
                      Active {activeDomains === 1 ? "domain" : "domains"}
                    </span>
                    ...
                    return (
                      ...
                      >
                        <div className={styles.categoryCardTop}>
                          <CategoryGlyph slug={cat.slug} label={cat.name} prominent={isPrimary} />
                          <span className={styles.noteCount}>{formatNoteCount(cat.noteCount)}</span>
                        </div>

                        <div className={styles.categoryCardBody}>
                          {isPrimary ? <p className={styles.cardEyebrow}>Most active category</p> : null}
      ...
      <section className={styles.section} id="featured-section">
        <div className={styles.container}>
          <div className={styles.sectionHeader}>
            ...
            <Link href="/browse?sort=popular" className="btn btn-secondary btn-sm">View all</Link>
          </div>
          ...
          <div className={styles.sectionShell}>
            {trendingNotes.length > 0 ? (
              <>
                <div className={styles.featuredSummary}>
                  ...
                  <div className={styles.summaryCard}>
                    ...
                    <span className={styles.summaryValue}>{latestTrendingNote ? `${latestTrendingNote.viewCount.toLocaleString()} ${latestTrendingNote.viewCount === 1 ? "view" : "views"}` : "N/A"}</span>
                  </div>
                </div>
                      ...
                      return (
                        <article key={note.id} className={styles.noteCard}>
                          ...
                          <div className={styles.noteMedia}>
                            {(() => {
                              const isUsableThumbnail = note.thumbnailUrl && !note.thumbnailUrl.includes("placeholder") && !note.thumbnailUrl.startsWith("/api/files/");
                              return isUsableThumbnail ? (
                                <Image src={note.thumbnailUrl} alt={note.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className={styles.noteMediaImage} />
                              ) : (
                                <div className={styles.noteMediaOg} style={{ backgroundImage: `url(/api/og?title=${encodeURIComponent(note.title)}&category=${encodeURIComponent(note.category)}&v=3)` }} />
                              );
                            })()}
                          </div>
                          <div className={styles.noteCardContent}>
                            ...
                            <h3 className={styles.noteTitle}><span>{note.title}</span></h3>
                            <p className={styles.noteDesc}>{note.description}</p>
                            <div className={styles.noteTags}>
                              {note.tags.slice(0, 3).map((tag: string) => <span key={tag} className={styles.noteTag}>#{tag}</span>)}
                            </div>
                            <div className={styles.noteMetrics}>
                              <span className={styles.noteMetric}>{note.bookmarkCount} {note.bookmarkCount === 1 ? "save" : "saves"}</span>
                            ...
                            <div className={styles.noteFooter}>
                              <div className={styles.noteAuthorWrap}>
                                <Link href={authorUrl} className={styles.noteAuthorGroup}>
                                  <span className={styles.noteAuthorAvatar} aria-hidden="true">{note.author.charAt(0).toUpperCase()}</span>
              ...
              <div className={styles.sectionEmpty}>
                ...
                <p className={styles.sectionEmptyText}>Notes that cross {viewsThreshold.toLocaleString()} {viewsThreshold === 1 ? "view" : "views"} will automatically show up in this section.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <section className={styles.ctaSection} id="cta-section">
        <div className={styles.container}>
          <div className={styles.ctaCard}>
            ...
            <p className={styles.ctaSubtitle}>Contribute your handwritten programming notes and help thousands of developers learn. You keep your copyright — we amplify your reach.</p>
            <div className={styles.ctaActions}>
              ...
              <Link href="/terms" id="cta-learn-btn" className="btn btn-secondary btn-lg">Learn about our ToS</Link>
            </div>
          </div>
        </div>
      </section>
